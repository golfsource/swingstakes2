// --- MULTI-PAGE SAFETY PATCH --------------------------------
// Safe element event binder -- silently skips if element not on this page
...
function getCurrentTab() {
  ...
  const map = {
    'index.html':'home','setup.html':'setup','score.html':'score',
    'board.html':'board','rounds.html':'rounds','events.html':'tourneys',
    'bets.html':'bets','stats.html':'stats','':`home`
  };
...
function $(id) {
  const el = document.getElementById(id);
  if (el) return el;
  // Return a safe no-op proxy so property sets/gets never throw on pages
  // that don't have this element (multi-page split).
  return new Proxy({}, {
    get(_, prop) {
      if (prop === 'style') return new Proxy({}, { set() { return true; }, get() { return ''; } });
      if (prop === 'classList') return { add(){}, remove(){}, toggle(){}, contains(){ return false; } };
      if (prop === 'dataset') return new Proxy({}, { get(){ return undefined; }, set(){ return true; } });
    ...
    set() { return true; } // silently ignore all property sets
  });
}
function _set(id, prop, val) { const el = document.getElementById(id); if (el) el[prop] = val; }
function fmt(n) {
  ...
  if (g.games.banker) {
    ...
    if (isLoserPickHole && !pickMade) {
      ...
      if ($('banker-clear-pick')) {
        { const _mel1 = $('banker-clear-pick'); if (_mel1) _mel1.onclick = async () => {
...
async function loadRounds() {
  ...
  { const _mel2 = $('btn-clear-all-rounds'); if (_mel2) _mel2.onclick = async () => {
...
function enterEditLiveRoundMode() {
  ...
  { const _dc=document.getElementById('display-code'); if(_dc) _dc.textContent = g.code; }
  { const _dc2=document.getElementById('display-code'); if(_dc2) _dc2.dataset.code = g.code; }
...
async function init() {
  ...
  document.querySelectorAll('.tab').forEach(t => {
    t.onclick = (e) => { if (t.getAttribute('aria-disabled')==='true') e.preventDefault(); };
  });

  // Quick actions on home
  { const _mel3 = $('btn-quick-new'); if (_mel3) _mel3.onclick = () => {
    switchTab('setup');
    $('btn-new-game').click();
  };
  }
  _set('btn-quick-join', 'onclick', () => switchTab('setup'))

  // New game button
  { const _mel4 = $('btn-new-game'); if (_mel4) _mel4.onclick = () => {
    ...
    { const _dc3=document.getElementById('display-code'); if(_dc3) _dc3.textContent = code; }
    $('display-code').dataset.code = code;
  };
  }

  { const _mel5 = $('btn-cancel-setup'); if (_mel5) _mel5.onclick = () => {
    if (state.editingLiveRound) {
      exitEditLiveRoundMode();
      switchTab('score');
      return;
  ...
  _set('btn-add-player', 'onclick', () => addPlayerRow())
  _set('btn-pick-roster', 'onclick', () => openRosterPicker())

  { const _mel6 = $('btn-add-tee'); if (_mel6) _mel6.onclick = () => {
    addTeeRow('', null, null);
    renderPlayerTeeSelects();
  };
  }
  ...
  _set('stats-player-select', 'onchange', () => renderStatsForSelectedPlayer())
  { const _mel7 = $('btn-set-me'); if (_mel7) _mel7.onclick = async () => {
  ...
  // Live slider label
  { const _mel8 = $('hcp-pct'); if (_mel8) _mel8.oninput = (e) => {
    { const _hpl=document.getElementById('hcp-pct-label'); if(_hpl) _hpl.textContent = e.target.value + '%'; }
  };
  }

  // Roster modal wiring
  _set('roster-cancel', 'onclick', () => $('roster-modal').classList.remove('show'))
  { const _mel9 = $('roster-modal'); if (_mel9) _mel9.onclick = (e) => {
    if (e.target.id === 'roster-modal') $('roster-modal').classList.remove('show');
  };
  }
  _set('roster-search', 'oninput', () => renderRosterList())
  ...
  { const _mel10 = $('btn-default-si'); if (_mel10) _mel10.onclick = () => {
    $('si-grid').querySelectorAll('input').forEach((inp, i) => inp.value = DEFAULT_SI[i]);
  };
  }
  ...
  // Game toggles
  ['skins','nassau','stroke','banker','vegas','dvegas','sixes','wolf','match','team-match','team-lowball','p3greenie','junk'].forEach(g => {
    const _gtoggle = $('game-' + g); if (!_gtoggle) return;
    _gtoggle.onchange = (e) => {
  ...
  { const _mel11 = $('btn-add-match-instance'); if (_mel11) _mel11.onclick = () => {
    if (!Array.isArray(state.matchInstances)) state.matchInstances = [];
    state.matchInstances.push({ value: 5, net: true, pair: [] });
    renderMatchInstances();
  };
  }
  { const _mel12 = $('btn-add-nassau-instance'); if (_mel12) _mel12.onclick = () => {
    if (!Array.isArray(state.nassauInstances)) state.nassauInstances = [];
    state.nassauInstances.push({
      value: 5, format: 'stroke', allowHuckle: true,
      birdiePay: 10, eaglePay: 20, hioPay: 100, pair: []
  ...
  // Start round
  { const _mel13 = $('btn-start-round'); if (_mel13) _mel13.onclick = async () => {
  ...
  { const _mel14 = $('btn-save-edits'); if (_mel14) _mel14.onclick = async () => {
    const g = state.game;
    if (!g) {
  ...
  { const _mel15 = $('btn-save-tourney'); if (_mel15) _mel15.onclick = () => {
    const setup = gatherSetup();
    if (!setup) return; // gatherSetup shows its own toast on validation errors
  ...
  { const _mel16 = $('stm-cancel'); if (_mel16) _mel16.onclick = () => {
    $('save-tourney-modal').classList.remove('show');
    state.pendingTourneySetup = null;
  };
  }

  { const _mel17 = $('save-tourney-modal'); if (_mel17) _mel17.onclick = (e) => {
    if (e.target.id === 'save-tourney-modal') $('save-tourney-modal').classList.remove('show');
  };
  }

  { const _mel18 = $('stm-save'); if (_mel18) _mel18.onclick = async () => {
  ...
  // Join existing
  { const _mel19 = $('btn-join-game'); if (_mel19) _mel19.onclick = async () => {
  ...
  // Hole nav
  { const _mel20 = $('prev-hole'); if (_mel20) _mel20.onclick = () => {
    if (state.currentHole > 1) { state.currentHole--; renderScoreEntry(); }
  };
  }
  { const _mel21 = $('next-hole'); if (_mel21) _mel21.onclick = () => {
    if (state.currentHole < 18) { state.currentHole++; renderScoreEntry(); }
  };
  }

  // Auto-advance button
  { const _mel22 = $('auto-advance-btn'); if (_mel22) _mel22.onclick = () => {
  ...
  { const _mel23 = $('btn-refresh'); if (_mel23) _mel23.onclick = async () => {
  ...
  _set('btn-end-round', 'onclick', () => switchTab('board'))
  _set('btn-back-score', 'onclick', () => switchTab('score'))

  { const _mel24 = $('btn-end-early'); if (_mel24) _mel24.onclick = () => {
    const g = state.game;
    if (!g) return;
    ...
    if ($('btn-end-early') && $('btn-end-early').dataset.mode === 'finish') {
  ...
  { const _mel25 = $('btn-finish-round'); if (_mel25) _mel25.onclick = async () => {
    const g = state.game;
    if (!g) {
  ...
  { const _mel26 = $('btn-reopen-round'); if (_mel26) _mel26.onclick = async () => {
    const g = state.game;
  ...
  { const _mel27 = $('summary-back'); if (_mel27) _mel27.onclick = () => {
    $('summary-modal').classList.remove('show');
  };
  }

  // Summary modal: "Save & return home" -- archive the round and go home
  { const _mel28 = $('summary-confirm'); if (_mel28) _mel28.onclick = async () => {
    const g = state.game;
    if (g) {
  ...
  { const _mel29 = $('summary-modal'); if (_mel29) _mel29.onclick = (e) => {
  ...
  { const _mel30 = $('btn-new-tourney'); if (_mel30) _mel30.onclick = () => {
    state.editingTourneyId = null;
    state.pendingTourneySetup = null;
    $('setup-no-game').style.display = 'none';
    $('setup-with-game').style.display = 'block';
  ...
  { const _mel31 = $('tm-cancel'); if (_mel31) _mel31.onclick = () => {
    $('tourney-modal').classList.remove('show');
    state.editingTourneyId = null;
  };
  }
  $('tm-save').onclick = saveTourney;
  { const _mel32 = $('tourney-modal'); if (_mel32) _mel32.onclick = (e) => {
  ...
  { const _mel33 = $('tam-cancel'); if (_mel33) _mel33.onclick = () => {
    $('tourney-actions-modal').classList.remove('show');
    state.activeTourneyForActions = null;
  };
  }
  { const _mel34 = $('tourney-actions-modal'); if (_mel34) _mel34.onclick = (e) => {
  ...
  { const _mel35 = $('tam-start'); if (_mel35) _mel35.onclick = async () => {
  ...
  { const _mel36 = $('tam-edit'); if (_mel36) _mel36.onclick = () => {
    const t = state.activeTourneyForActions;
    if (!t) return;
    $('tourney-actions-modal').classList.remove('show');
    openTourneyModal(t);
  ...
  { const _mel37 = $('tam-delete'); if (_mel37) _mel37.onclick = async () => {
    const t = state.activeTourneyForActions;
    if (!t) return;
  ...
  // Wolf modal
  _set('wolf-cancel', 'onclick', () => $('wolf-modal').classList.remove('show'))
  { const _mel38 = $('wolf-modal'); if (_mel38) _mel38.onclick = (e) => {
    if (e.target.id === 'wolf-modal') $('wolf-modal').classList.remove('show');
  };
  }

  // Huckle modal
  _set('huckle-cancel', 'onclick', () => $('huckle-modal').classList.remove('show'))
  { const _mel39 = $('huckle-modal'); if (_mel39) _mel39.onclick = (e) => {
...
// -- PAGE-AWARE ACTIVATION ----------------------------------
window.addEventListener('DOMContentLoaded', () => {
  const cur = getCurrentTab();
  document.querySelectorAll('.tab').forEach(t => {
    const href = (t.getAttribute('href') || '').split('/').pop();
    t.classList.toggle('active', href === (PAGE_MAP[cur] || 'index.html'));
  });
